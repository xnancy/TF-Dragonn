...
class Dataset(object):
    ...
    def __init__(self, feature_beds=None,
        ...
        if self.has_feature_beds_and_regions_or_labels:
            raise ValueError(
                "Invalid Dataset: includes features beds and regions and/or labels!")
        if self.has_raw_and_encoded_dnase:
            raise ValueError(
                "Invalid Dataset: includes raw and encoded dnase!")
        if self.has_raw_and_encoded_fasta:
            raise ValueError(
                "Invalid Dataset: includes raw and encoded fasta!")
...
class IntervalDataset(Dataset):
    ...
    def __init__(self, **kwargs):
        ...
        if not self.has_valid_intervals:
            raise ValueError(
                "Invalid IntervalDataset: must have either cached intervals or raw intervals!")
...
class LabeledIntervalDataset(IntervalDataset):
    ...
    def __init__(self, **kwargs):
        ...
        if not self.has_valid_labels:
            raise ValueError(
                "Invalid LabeledIntervalDataset: must have either labels for regions or feature_beds!")
...
class OrderedLabeledIntervalDataset(LabeledIntervalDataset):
    ...
    def __init__(self, **kwargs):
        ...
        if not self.has_ordered_labels:
            raise ValueError(
                "Invalid OrderedLabeledIntervalDataset: must have either labels for regions or feature_beds list!")
...
class Datasets(object):
    ...
    def __init__(self, dataset_dict, task_names=None):
        ...
        if not self.has_consistent_datasets:
            raise ValueError(
                "Datasets are inconsistent: multiple dataset types are not allowed in the same config file!")
    ...
    def check_nonordered_labeled_interval_datasets(self):
        ...
        assert len(task_names) == len(
            self.task_names), "task names must be unique!"
        for dataset_id, dataset in self:
            assert type(dataset.feature_beds) is collections.OrderedDict, "feature beds in dataset {} are not a dictionary:\n{}".format(
                dataset_id, dataset.feature_beds)
            dataset_task_names = set(dataset.feature_beds.keys())
            assert dataset_task_names.issubset(task_names), "Tasks {} in {} are not in task_names!".format(
                dataset_task_names - task_names, dataset_id)
            if dataset.ambiguous_feature_beds is not None:
                assert type(dataset.ambiguous_feature_beds) is collections.OrderedDict, "ambiguous feature beds in dataset {} are not a dictionary:\n{}".format(
                    dataset_id, dataset.ambiguous_feature_beds)
                dataset_ambiguous_task_names = set(
                    dataset.ambiguous_feature_beds.keys())
                assert dataset_ambiguous_task_names.issubset(task_names), "Tasks {} in {} are not in task_names!".format(
                    dataset_ambiguous_task_names - task_names, dataset_id)
    ...
    def convert_to_ordered_labeled_interval_datasets(self):
        ...
        for i, (dataset_id, dataset) in enumerate(self):
            ...
            for task_name in self.task_names:
                feature_beds_list.append(dataset.feature_beds[
                                         task_name] if task_name in dataset.feature_beds.keys() else None)
            self.datasets[i].feature_beds = feature_beds_list
            if dataset.ambiguous_feature_beds is not None:
                ambiguous_feature_beds_list = []
                for task_name in self.task_names:
                    ambiguous_feature_beds_list.append(dataset.ambiguous_feature_beds[
                                                       task_name] if task_name in dataset.ambiguous_feature_beds.keys() else None)
                self.datasets[
                    i].ambiguous_feature_beds = ambiguous_feature_beds_list
            else:
        ...
        if self.memmaped:
            ...
            if self.memmaped_fasta:
                ...
                for dataset_id, dataset in self:
                    if dataset.genome_data_dir not in genome_data_dir2fasta_extractor:
                        genome_data_dir2fasta_extractor[
                            dataset.genome_data_dir] = MemmappedFastaExtractor(dataset.genome_data_dir)
                    dataset2extractors[dataset_id].append(
                        genome_data_dir2fasta_extractor[dataset.genome_data_dir])
            if self.memmaped_dnase:
                ...
                for dataset_id, dataset in self:
                    ...
                    dataset2extractors[dataset_id].append(
                        dnase_data_dir2bigwig_extractor[dataset.dnase_data_dir])
        else:
            raise RuntimeError(
                "get_dataset2extractors doesn't support non memmaped inputs!")
...
class RawInputsConfig(Config):
    ...
    def __init__(self, dataset_id2raw_inputs):
        ...
        if rv is True:
            ...
            raise ValueError("Datasets {} and {} have inconsistent genome fasta types!".format(
        ...
        if rv is True:
            ...
            dataset_id1, dataset_id2 = rv
            raise ValueError("Datasets {} and {} have inconsistent dnase bigwig types!".format(
                dataset_id1, dataset_id2))
...
class ProcessedInputsConfig(Config):
    ...
    def __init__(self, dataset_id2processed_inputs):
        ...
        if rv is True:
            ...
            dataset_id1, dataset_id2 = rv
            raise ValueError("Datasets {} and {} have inconsistent genome data dir types!".format(
                dataset_id1, dataset_id2))
        ...
        if rv is True:
            ...
            dataset_id1, dataset_id2 = rv
            raise ValueError("Datasets {} and {} have inconsistent dnase data dir types!".format(
                dataset_id1, dataset_id2))
...
def parse_raw_inputs_config_file(raw_input_config_fname):
    ...
    """
    dataset_id2raw_inputs = json.load(
        open(raw_input_config_fname), object_pairs_hook=collections.OrderedDict)
...
def parse_processed_inputs_config_file(processed_input_config_fname):
    ...
    Returns instance of RawInputsConfig
    """
    dataset_id2processed_inputs = json.load(
        open(processed_input_config_fname), object_pairs_hook=collections.OrderedDict)
    for dataset_id, processed_inputs in dataset_id2processed_inputs.items():
        dataset_id2processed_inputs[
            dataset_id] = ProcessedInputs(**processed_inputs)
    ...
    def __init__(self, dataset_id2raw_intervals, task_names):
        ...
        for dataset_id, raw_intervals in self:
            assert type(raw_intervals.feature_beds) is collections.OrderedDict, "feature beds in dataset {} are not a dictionary:\n{}".format(
                dataset_id, raw_intervals.feature_beds)
            ...
            if raw_intervals.ambiguous_feature_beds is not None:
                assert type(raw_intervals.ambiguous_feature_beds) is collections.OrderedDict, "ambiguous feature beds in dataset {} are not a dictionary:\n{}".format(
                    dataset_id, raw_intervals.ambiguous_feature_beds)
                dataset_ambiguous_task_names = set(
                    raw_intervals.ambiguous_feature_beds.keys())
                assert dataset_ambiguous_task_names.issubset(task_names), "Tasks {} in {} are not in task_names!".format(
                    dataset_ambiguous_task_names - task_names, dataset_id)
        ...
        # converts dictionaries of feature and ambiguous beds to uniformly
        # ordered lists of files
        for i, (dataset_id, raw_intervals) in enumerate(self):
            feature_beds_list = []
            for task_name in self.task_names:
                feature_beds_list.append(raw_intervals.feature_beds[
                                         task_name] if task_name in raw_intervals.feature_beds.keys() else None)
            self.raw_intervals_list[i].feature_beds = feature_beds_list
            if raw_intervals.ambiguous_feature_beds is not None:
                ambiguous_feature_beds_list = []
                for task_name in self.task_names:
                    ambiguous_feature_beds_list.append(raw_intervals.ambiguous_feature_beds[
                                                       task_name] if task_name in raw_intervals.ambiguous_feature_beds.keys() else None)
                self.raw_intervals_list[
                    i].ambiguous_feature_beds = ambiguous_feature_beds_list
            else:
...
def parse_raw_intervals_config_file(raw_intervals_config_file):
    ...
    """
    dataset_id2raw_intervals = json.load(
        open(raw_intervals_config_file), object_pairs_hook=collections.OrderedDict)
...
class ProcessedIntervalsConfig(Config):
    ...
    def __init__(self, dataset_id2processed_intervals, task_names):
        ...
        if rv is True:
            ...
            dataset_id1, dataset_id2 = rv
            raise ValueError("Datasets {} and {} have inconsistent presence of labels!".format(
                dataset_id1, dataset_id2))
...
def parse_processed_intervals_config_file(processed_intervals_config_file):
    ...
    """
    dataset_id2processed_intervals = json.load(
        open(processed_intervals_config_file), object_pairs_hook=collections.OrderedDict)
    for dataset_id, processed_intervals in dataset_id2processed_intervals.items():
        if dataset_id == "task_names":
            ...
            continue
        dataset_id2processed_intervals[
            dataset_id] = ProcessedIntervals(**processed_intervals)
